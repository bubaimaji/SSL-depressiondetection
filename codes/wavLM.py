import pandas as pd
from transformers import Wav2Vec2Processor, Wav2Vec2Model,Wav2Vec2FeatureExtractor, WavLMModel
...
full_model = WavLMModel.from_pretrained("microsoft/wavlm-base")

#from pydub import AudioSegment
#def normalize_audio(audio_path):
    #audio = AudioSegment.from_file(audio_path)
    #normalized_audio = audio.apply_gain(-audio.max_dBFS)  # Normalize to 0 dBFS
    #return np.array(normalized_audio.get_array_of_samples(), dtype=np.float32) / 2**15  # Convert to float32 array


def extract_features(model, audio_paths, target_sampling_rate=16000, batch_size=10):
    batch_features = []
